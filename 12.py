...
print("Напишите кол-во кустов (N):")
N = int(input())
A = list()

for i in range(N):
    print("Напишите, сколько ягод находится на кусте ",i+1)
    A.append(int(input()))

print("Ягод на каждом кусте:",A)

arr_count = list()
for i in range(len(A) - 1):
    arr_count.append(A[i-1] + A[i] + A[i+1])
arr_count.append(A[-2] + A[-1]+ A[0])
print("Максимальное кол-во ягод, которых можно собрать за один прогон:",max(arr_count))